<template>
  <v-card class="service-card h-100" elevation="4" hover @click="handleServiceClick">
    <div class="card-image-container">
      <v-img 
        :src="service.image || service.img_url" 
        :alt="service.name" 
        height="200" 
        cover 
        class="card-image"
      >
        ...
        <template #error>
          <div class="error-placeholder">
            <v-icon size="60" color="white">{{ service.icon || 'mdi-account-tie' }}</v-icon>
          </div>
        </template>
      </v-img>
    ...
    <v-card-text class="service-description">
      {{ service.shortDescription || service.short_description || service.description || 'Professional service' }}
    </v-card-text>
...
const props = defineProps({
  ...
  isDirectorService: { type: Boolean, default: false }
})
...
const handleLearnMore = () => {
  ...
  const route = props.isDirectorService 
    ? `/director-services/${serviceId}` 
    : `/services/${serviceId}`
