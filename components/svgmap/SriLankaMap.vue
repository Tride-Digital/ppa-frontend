...
onMounted(() => {
  ...
  if (typeof (window as any).Raphael === 'undefined') {
...
async function loadProvinceDirectors() {
  const provinces = Object.keys(provinceColors)
  ...
  for (const province of provinces) {
    try {
      ...
      if (directors && directors.length > 0) {
        ...
        const title = getDirectorTitle(directorId)
...
function initMap() {
  ...
  // Define all other districts... (keeping the same structure)
  const mahanuvara = rsr.path(
  ...
  // Group districts into province sets
  const western = rsr.set().push(colombo, gampaha, kalutara)
  const central = rsr.set().push(mahanuvara, matale, nuwaraeliya)
  const southern = rsr.set().push(galle, matara, hambantota)
  const northern = rsr.set().push(jaffna, kilinochchi, mannar, vavuniya, mullaitivu)
  const eastern = rsr.set().push(batticaloa, ampara, trincomalee)
  const northWestern = rsr.set().push(kurunegala, puttalam)
  const northCentral = rsr.set().push(anuradhapura, polonnaruwa)
...
const provinceSets = [
    ...
    { set: northern, name: 'Northern', route: 'Northern' },
    { set: eastern, name: 'Eastern', route: 'Eastern' },
    { set: northWestern, name: 'North Western', route: 'North-Western' },
    { set: northCentral, name: 'North Central', route: 'North-Central' },
    { set: uva, name: 'Uva', route: 'Uva' },
  ...
  provinceSets.forEach((province) => {
    ...
    province.set.click(function () {
      // Navigate to director profile with province parameter
...
async function navigateToDirector(provinceName: string) {
  ...
  try {
    // Fetch the primary director ID for this province
    ...
    // Fetch director info to get title
    await fetchDirectorInfo(directorId)
    const title = getDirectorTitle(directorId)
    ...
    console.error('Error navigating to director:', error)
  } finally {
