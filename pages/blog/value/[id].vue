<template>
  <div v-if="blogPost">
    <!-- Value Addition Blog Detail Page -->
    <div class="blog-detail-page value-theme">
      <!-- Hero Section -->
          ...
          <v-row justify="center">
            ...
            <v-col cols="12" lg="8" xl="8">
              <article class="article-content">
                ...
                <!-- Process Stage Badge -->
                <div class="process-stage-section mb-6">
                  <v-chip
                    color="primary"
                    variant="outlined"
                    size="large"
                ...
                <div class="tags-section mt-8 pt-6">
                  <h3 class="tags-title mb-4">Value Addition Topics</h3>
      ...
      <section class="comments-section py-8 bg-grey-lighten-5">
        <v-container>
          <v-row justify="center">
            <v-col cols="12" lg="8" xl="8">
              <h2 class="comments-title mb-6">Value Creation Discussion ({{ comments.length }})</h2>
              
              <!-- Comment Form -->
              <v-card class="comment-form mb-6" elevation="2">
                <v-card-title>
                  <v-icon class="me-2">mdi-comment-plus</v-icon>
                  Share Your Value Addition Experience
                </v-card-title>
                <v-card-text>
                  <v-textarea
                    v-model="newComment"
                    label="Share your experience with value addition processes..."
                    variant="outlined"
                    rows="4"
                    class="mb-4"
                  />
  ...
  <div v-else-if="pending" class="loading-container">
    <v-container class="d-flex justify-center align-center" style="min-height: 50vh;">
      <v-progress-circular
        ...
        color="primary"
      />
      <span class="ml-4">Loading value addition article...</span>
    </v-container>
  </div>
  ...
  <div v-else class="error-container">
    <v-container class="text-center" style="min-height: 50vh;">
      ...
      <h2 class="mb-4">Value Addition Article Not Found</h2>
      <p class="mb-6">The value addition article you're looking for doesn't exist or has been removed.</p>
...
// Use the VALUE ADDITION store
const { getPostById, getRelatedPosts } = useValueAdditionStore()
...
onMounted(async () => {
  try {
    ...
    if (post) {
      ...
      // Update page meta
      useHead({
        title: `${post.title} - Value Addition | PPA Blog`,
        meta: [
    ...
    console.error('Error loading value addition post:', error)
  } finally {
    pending.value = false
  }
})
...
// Sample comments for value addition discussions
const comments = ref([
  {
    id: 1,
    author: 'Insha Asif',
...
const navigateToPost = async (postId) => {
  try {
    await router.push(`/blog/value/${postId}`)
  } catch (error) {
